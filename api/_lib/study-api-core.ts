...
// Local dev / vercel-dev convenience. On Vercel production, env vars are injected by platform.
loadServerEnv();
...
const DEFAULT_DATA_DIR = process.env.VERCEL
  ? path.join('/tmp', '.english-interaction-study')
  : path.join(os.homedir(), '.english-interaction-study');
const DATA_DIR = process.env.STUDY_DATA_DIR || DEFAULT_DATA_DIR;
...
function ensureLocalDataFile() {
  if (PERSISTENCE_MODE !== 'local-json') return;
...
export async function handleSessionSync(body: any) {
  const { session } = body || {};
  const devOverrideCondition = body?.devOverrideCondition === true;
  if (!session || !isValidSessionId(session.sessionId)) {
    return { status: 400, payload: { error: 'Missing or invalid session' } };
  }
  ...
  if (!assignedCondition && session.consent === true) {
    if (devOverrideCondition && process.env.VITE_ENABLE_DEV_TOOLS === 'true' && isValidCondition(session.condition)) {
      assignedCondition = session.condition;
      console.log(`[sync] dev override condition applied for ${session.sessionId}: ${assignedCondition}`);
    } else {
  ...
  return { status: 200, payload: { success: true, condition: assignedCondition || null } };
}

export async function handleChat(body: any) {
  const { sessionId, history, message } = body || {};
  if (!isValidSessionId(sessionId)) {
    return { status: 400, payload: { error: 'Invalid sessionId' } };
  }
  ...
  if (!storedSession) {
    return { status: 404, payload: { error: 'Unknown sessionId' } };
  }
  ...
  if (!condition) {
    return { status: 409, payload: { error: 'Session has no assigned condition yet' } };
  }
  ...
  if (!getOpenAIApiKey()) {
    if (!isMockChatEnabled()) {
      ...
      return {
        ...
        payload: {
          error:
            'Server misconfigured: OPENAI_API_KEY is missing. Configure it or explicitly enable mock mode with ENABLE_MOCK_CHAT=true.',
        },
      };
    }
    ...
    let correction = '';
    let reply =
      'This is a mock response because OPENAI_API_KEY is not configured yet. In a real session, the bot will respond based on your English learning experience. How do you feel about speaking English in public?';
